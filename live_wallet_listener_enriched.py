...
class ListenerState:
    def __init__(self, wallet: str, out_path: Path, seen_path: Path, audit_path: Path) -> None:
        self.wallet = wallet
...
def enrich_transaction(tx: Dict[str, Any], wallet: str, signature: str, observed_utc: str, rpc: RpcClient) -> Dict[str, Any]:
    meta = (tx or {}).get("meta") or {}
    msg = ((tx or {}).get("transaction") or {}).get("message") or {}
...
def process_signature(state: ListenerState, rpc: RpcClient, signature: str, observed_utc: str) -> None:
    ...
    enriched = enrich_transaction(tx, state.wallet, signature, observed_utc, rpc)
...
def pick_endpoints(api_key: str, ws_override: Optional[str]) -> Tuple[str, Optional[str]]:
    if api_key:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Live enriched wallet listener for Solana")
    p.add_argument("--wallet", default=DEFAULT_WALLET, help="wallet pubkey to track")
    p.add_argument("--out", default=DEFAULT_OUT, help="append-only JSONL output path")
    p.add_argument("--seen", default=DEFAULT_SEEN, help="seen-signature file path")
    p.add_argument("--audit", default=DEFAULT_AUDIT, help="audit sidecar JSON path")
    p.add_argument("--ws", default=None, help="optional websocket endpoint override")
...
def main() -> None:
    ...
    rpc_url, ws_url = pick_endpoints(api_key, args.ws)
    ...
    print(f"[info] wallet={args.wallet}", flush=True)
    ...
    ws_ok = False
    if ws_url:
        ws_ok = try_ws_listen_with_websocket_client(state, ws_url, args.wallet)
        if not ws_ok:
            ws_ok = try_ws_listen_with_websockets(state, ws_url, args.wallet)
